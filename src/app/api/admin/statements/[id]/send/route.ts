...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (via.includes("sms") && customer.phone) {
    ...
    await sendSms(customer.phone, `Eastern LM — Your statement balance is ${fmt(stmt.balance_due_cents as number)}. Pay online: ${process.env.NEXT_PUBLIC_SITE_URL ?? "https://easternlm.com"}/pay/${id}\n\nReply STOP to opt out.`).catch(() => {});
  }
