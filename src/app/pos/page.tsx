      ...
      {showCheckout && (
        <CheckoutOverlay
          ...
          onProcessCard={async (amountCents) => {
            // Pre-charge validation for DELIVERY orders only — name, phone, and
            // address are required server-side, so verify before charging the card.
            // Pickup (walk-in) charges don't require customer info.
            if (deliveryMethod === "delivery") {
              const effectiveName = (delName || customerName || "").trim();
              const effectivePhone = (delPhone || customerPhone || "").trim();
              const effectiveAddress = (delAddress || deliveryAddress || "").trim();
              const missing: string[] = [];
              if (!effectiveName || effectiveName === "Walk-in") missing.push("customer name");
              if (!effectivePhone) missing.push("phone number");
              if (!effectiveAddress) missing.push("delivery address");
              if (missing.length > 0) {
                const msg = `Delivery card payments require ${missing.join(", ")}. Add delivery info, then try again.`;
                alert(msg);
