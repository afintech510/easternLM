    ...
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-12 md:py-16">
      <section className="space-y-4">
        ...
        <p className="max-w-3xl text-muted-foreground">
          Live catalog source: <span className="font-semibold">{catalog.source}</span>.{" "}
            ...
            <div className="mt-3 space-y-2">
              <Link
                ...
                className={`block rounded-lg px-3 py-2 text-sm ${
                  !selectedCategory ? "bg-primary text-primary-foreground" : "hover:bg-background"
                }`}
              >
                All Materials
              </Link>
              {catalog.categories.map((category) => (
                <Link
                  ...
                  className={`block rounded-lg px-3 py-2 text-sm ${
                    selectedCategory === category.slug ? "bg-primary text-primary-foreground" : "hover:bg-background"
                  }`}
                >
                  {category.name}
                </Link>
            ...
            <div className="mt-3 space-y-2">
              {sortOptions.map((option) => (
                <Link
                  ...
                  className={`block rounded-lg px-3 py-2 text-sm ${
                    selectedSort === option.value ? "bg-primary text-primary-foreground" : "hover:bg-background"
                  }`}
                >
                  {option.label}
                </Link>
              ...
              {catalog.products.map((product) => (
                <article key={product.id} className="rounded-2xl border bg-card p-4">
                  <Link href={`/shop/${product.slug}`} className="block">
                    <Image
                      src={product.images[0] ?? "https://via.placeholder.com/800x500?text=Product"}
                      alt={product.name}
                  ...
                  <Link href={`/shop/${product.slug}`} className="mt-1 block text-base font-semibold hover:text-primary">
                    {product.name}
                  </Link>
