  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold">Product Feed</h2>
          <p className="text-sm text-muted-foreground">
            Google Merchant Center product sync status
          </p>
        </div>
        <FeedSyncButton />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-lg border p-4">
          ...
          <p className="text-2xl font-bold text-red-600">{disapproved}</p>
        </div>
      </div>
      ...
      {total === 0 ? (
        <div className="rounded-lg border p-8 text-center text-muted-foreground">
          ...
          <p className="text-sm mt-1">
            Click &ldquo;Sync from GMC&rdquo; to pull product statuses from Google Merchant Center.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                <th className="px-4 py-2 text-left font-medium">Offer ID</th>
            ...
            <tbody>
              {(products || []).map((p: any) => (
                <tr key={`${p.gmc_offer_id}-${p.channel}`} className="border-b last:border-0">
                  <td className="px-4 py-2 font-mono text-xs">{p.gmc_offer_id}</td>
                  <td className="px-4 py-2">{p.channel}</td>
                  <td className="px-4 py-2">
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${
                        p.last_sync_status === "approved"
                          ? "bg-green-100 text-green-700"
                          : p.last_sync_status === "disapproved"
                            ? "bg-red-100 text-red-700"
                            : "bg-amber-100 text-amber-700"
                      }`}
                    >
                      {p.last_sync_status || "pending"}
                    </span>
                    {p.disapproval_reason && (
                      <p className="text-xs text-red-600 mt-0.5">{p.disapproval_reason}</p>
                    )}
                  </td>
