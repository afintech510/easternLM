/**
 * Generate Sales Tax Report — May 2026 (POS/web only, no WooCommerce merge).
 * Outputs: Sales_Tax_Report_May_2026.csv / .html in /app/.
 *
 * May 2026 is entirely on the new POS/web system (WC retired Mar 14, 2026),
 * so this is a single-source Supabase report. COD / pending / refunded excluded.
 */
const fs = require("fs");
...
(async () => {
  const mayRows = await fetchMonth("2026-05");
  ...
  const allRaw = mayRows.map(normalize);
  const rows = allRaw.filter((r) => !isPending(r) && !isRefunded(r) && !isCod(r));

  // Taxable Sales back-calculated from actual tax_cents: taxable = tax / 0.0875
  const RATE = 0.0875;
  for (const r of rows) {
  ...
  fs.writeFileSync("/app/Sales_Tax_Report_May_2026.csv", csvLines.join("\n") + "\n");
  ...
  const t = agg(rows);
  console.log(`May 2026: ${t.count} orders, taxable ${fmt(t.taxable)}, exempt ${fmt(t.exempt)}, tax ${fmt(t.tax)}, grand ${fmt(t.grand)}`);
...
<html><head><meta charset="utf-8"><title>Sales Tax Report — May 2026</title><style>${css}</style></head><body>
  <h1>Sales Tax Report</h1>
  <div class="meta">
    <strong>Eastern Landscape &amp; Mason Supply</strong> · 110 Frowein Road, Center Moriches, NY 11934<br/>
    Period: May 1 – May 31, 2026 · Generated ${new Date().toLocaleString("en-US", { dateStyle: "long", timeStyle: "short" })}<br/>
    NY State sales tax rate: 8.75% (Suffolk County)
  </div>

  <div class="grand-box">
    <div class="label">Total Sales Tax Collected — May 2026</div>
  ...
  fs.writeFileSync("/app/Sales_Tax_Report_May_2026.html", html);
